// src/dtos/headHeadTeacher/headHeadTeacher.zod.ts
import { z } from "zod";

// Base HeadTeacher DTO schema with all properties
const BaseHeadTeacherDtoZodSchema = z.object({
  // Basic Information
  school_uid: z.string().min(1, "School UID is required"),
  head_teacher_index: z.number().int().min(1, "HeadTeacher Index is required"),
...
// HeadTeacher Signup DTO schema
export const HeadTeacherSignupDtoZodSchema = BaseHeadTeacherDtoZodSchema.pick({
  school_uid: true,
  head_teacher_index: true,
  username: true,
  name: true,
  gender: true,
  nid_number: true,
...
// HeadTeacher Login DTO schema
export const HeadTeacherLoginDtoZodSchema = BaseHeadTeacherDtoZodSchema.pick({
  school_uid: true,
  username: true,
  password: true,
});

// HeadTeacher Update DTO schema
export const HeadTeacherUpdateDtoZodSchema =
  BaseHeadTeacherDtoZodSchema.partial().extend({
    school_uid: z.string().min(1, "School UID is required"),
  });
